...
const SOURCE_ROOT = resolve("updatedReports/ke-order-60-omit-zero-value-2026-08-05/reports")
const LEGACY_ORDER_ID = 60
const EXPECTED_ITEMS = [
  { name: "Tapal Green Tea Bags (30 PCS)", quantity: 1, priceCents: 24_500 },
  { name: "Tapal Family Mixture Tea (900 GM)", quantity: 1, priceCents: 178_000 },
  { name: "Tapal Family Mixture Tea (430 GM)", quantity: 1, priceCents: 89_500 },
  { name: "Tapal Danedar Teabags (600 PCS)", quantity: 1, priceCents: 329_500 },
  { name: "Tapal Danedar Teabags (100 PCS)", quantity: 2, priceCents: 98_000 },
] as const
...
async function main() {
  ...
  if (source.prepared.length !== 1 || expectedOrder?.legacyOrderId !== LEGACY_ORDER_ID || source.rejected.length !== 0) {
    throw new Error("Order 60 prepared-source validation failed")
  }
  ...
  try {
    ...
    if (order) {
      ...
      check(order.tid === "KE-LEGACY-60", "TID mismatch")
      ...
      check(Number(order.subtotal_cents) === 817_500 && Number(order.tax_cents) === 147_150 && Number(order.total_cents) === 964_650, "Order financial mismatch")
      check(Number(order.refund_amount_cents ?? 0) === 0, "Unexpected refund amount")
      check(order.source_payload?.sourceHeader?.OriginalStatusID === 9 && order.source_payload?.sourceHeader?.OriginalDeliveryStatus === 506, "Original status provenance mismatch")
      ...
      check(order.source_payload?.sourceHeader?.ItemTreatmentPolicy === "USER_APPROVED_OMIT_POSITIVE_QUANTITY_ZERO_VALUE_ITEM", "Item-policy provenance mismatch")
      check(JSON.stringify(order.source_payload?.sourceHeader?.OmittedLegacyItemRowIds) === JSON.stringify([110]), "Omitted-row provenance mismatch")
      check(Number(order.receipt_data?.subtotal) === 8175 && Number(order.receipt_data?.tax) === 1471.5 && Number(order.receipt_data?.totalAmount) === 9646.5, "Receipt totals mismatch")
      ...
      check(items.every((item) => Number(item.quantity) > 0 && Number(item.price_cents) > 0), "Zero/non-positive item leaked into imported order")
      check(items.reduce((sum, item) => sum + Math.round(Number(item.quantity) * Number(item.price_cents)), 0) === 817_500, "Imported item subtotal mismatch")
      ...
      for (const expected of EXPECTED_ITEMS) {
        ...
        if (matches[0]) {
          check(Number(matches[0].quantity) === expected.quantity && Number(matches[0].price_cents) === expected.priceCents, `${expected.name} quantity/price mismatch`)
        }
      }
    ...
    check(Number(countRows[0]?.count) === 705, `Expected 705 imported legacy orders; found ${countRows[0]?.count}`)
