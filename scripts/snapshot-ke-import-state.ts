...
async function main() {
  ...
  try {
    ...
    const queries: Record<string, { text: string; params?: unknown[] }> = {
      ...
      orders: { text: "select id, tid, organization_id, branch_id, status, fulfillment_status, payment_status, subtotal_cents, tax_cents, total_cents, created_by_user_id, created_at, delivered_at, fulfilled_at, refunded_at, refunded_by_user_id, status_at_refund, refund_amount_cents, refund_reason, updated_at, receipt_data from orders where organization_id = $1 order by id", params: [ORG_ID] },
      ...
      refunds: { text: "select id, organization_id, order_id, amount_cents, coalesce((to_jsonb(refunds)->>'tax_refund_cents')::bigint, 0) as tax_refund_cents, reason, status, refund_number, requested_by_user_id, processed_by_user_id, created_at, updated_at from refunds where organization_id = $1 order by id", params: [ORG_ID] },
      refundItems: { text: "select ri.* from refund_items ri join refunds r on r.id = ri.refund_id where r.organization_id = $1 order by ri.id", params: [ORG_ID] },
