...
async function main() {
  ...
  try {
    ...
    const queries: Record<string, { text: string; params?: unknown[] }> = {
      ...
      refunds: { text: "select id, organization_id, order_id, amount_cents, reason, status, refund_number, requested_by_user_id, processed_by_user_id, created_at, updated_at from refunds where organization_id = $1 order by id", params: [ORG_ID] },
