...
const PRE_PATH = resolve("backups/ke-import-state-2026-08-05-pre-order-60.json")
const POST_PATH = resolve("backups/ke-import-state-2026-08-05-post-order-60.json")
const OUTPUT_PATH = resolve("updatedReports/ke-order-60-omit-zero-value-2026-08-05/snapshot-diff.json")
...
function sidecar(path: string): string {
  return readFileSync(`${path}.sha256`, "utf8").trim().split(/\s+/)[0]
}
...
function main() {
  ...
  for (const snapshot of [pre, post]) {
    check(snapshot.organization?.[0]?.id === 10 && snapshot.organization?.[0]?.code === "0001" && snapshot.organization?.[0]?.name === "K-Electric" && snapshot.organization?.[0]?.status === "active", "K-Electric identity mismatch")
  }
  ...
  for (const name of ["branchInventory", "orders", "orderItems", "legacyImportBatches", "legacyOrderImports"]) {
  ...
  const expectedDeltas = {
    orders: 1, orderItems: 5, branchInventory: 4, batches: 1, imports: 1,
  ...
  check(newImports.length === 1 && Number(newImports[0]?.legacy_order_id) === 60 && Number(newImports[0]?.organization_id) === 10 && newImports[0]?.source_system === "KE_LOGISTICS", "New import ledger row mismatch")
  const importedOrder = newImports[0] ? (post.orders as Row[]).find((row) => Number(row.id) === Number(newImports[0].order_id)) : undefined
  check(importedOrder?.tid === "KE-LEGACY-60" && importedOrder?.status === "FULFILLED" && importedOrder?.fulfillment_status === "DELIVERED", "Imported order identity/status mismatch")
  check(Number(importedOrder?.subtotal_cents) === 817_500 && Number(importedOrder?.tax_cents) === 147_150 && Number(importedOrder?.total_cents) === 964_650, "Imported order money mismatch")

  const importedItems = importedOrder ? (post.orderItems as Row[]).filter((row) => Number(row.order_id) === Number(importedOrder.id)) : []
  check(importedItems.length === 5, `Expected five imported items; found ${importedItems.length}`)
  check(importedItems.every((row) => Number(row.quantity) > 0 && Number(row.price_cents) > 0), "Zero/non-positive imported item found")
  check(importedItems.reduce((sum, row) => sum + Math.round(Number(row.quantity) * Number(row.price_cents)), 0) === 817_500, "Imported item subtotal mismatch")
  ...
  check(post.legacyOrderImports.length === 705 && post.orders.length === 705, "Final imported/order count is not 705")
