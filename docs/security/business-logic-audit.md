...
- Refunds are itemized from order-item snapshot prices, do not depend on payment status, are limited to the order's calendar month, and are allowed for `PENDING`, `APPROVED`, or `FULFILLED` orders. Full refunds set `REFUNDED`; partial refunds preserve the prior order status.
- Non-Super-Admin refund actors create `PENDING` requests. Order Portal actors additionally require their own order to be `FULFILLED` with delivery status `DELIVERED`. Super Admin can approve/process or cancel requests. Approved and legacy `COMPLETED` records count against refundable money and quantity.
...
| Refund amount and remaining eligibility | Order-item price snapshots, ordered quantities, approved/pending refund rows, order total, and eligible order status |
...
| `PENDING` | `REFUNDED` | Same-month, full refund approved by Super Admin |
| `APPROVED` | `FULFILLED` | Head Office/Super Admin plus approval token, or system auto-fulfil after delivered window |
| `APPROVED` | `REFUNDED` | Same-month, full refund approved by Super Admin |
| `FULFILLED` | `REFUNDED` | Same-month, full refund approved by Super Admin |
...
- Refund transactions lock the order and recalculate status eligibility, prior quantities, approved/pending amounts, and remaining capacity before any write.
...
4. **Pending-order refunds.** Existing code explicitly allows refunding a `PENDING` order regardless of payment status. Confirm this remains intended instead of requiring rejection/cancellation first.
5. **Manual fulfilment shortcut.** Confirm that a token-authorized Head Office/Super Admin may move `APPROVED` directly to `FULFILLED`/`DELIVERED`, bypassing delivery-progress steps.
6. **Refund budget accounting.** One Super Admin refund path subtracts held/spent while the refund-management path adds budget credit. Both increase availability by the refund amount, but reporting semantics differ. Choose one accounting convention before consolidating.
7. **Maximum quantity.** Confirm the new 1,000,000-per-line safety ceiling or provide product/category-specific maxima.
8. **Order deletion.** Confirm that fulfilled, paid, rejected, or refunded orders must remain immutable financial history; the hardened endpoint now deletes only unpaid, refund-free pending/approved orders.
9. **Inactive settings.** `auto_approve_orders` and `order_approval_threshold` are configurable, and UI helpers describe a two-hour auto-approval window, but no server auto-approval mutation exists. Confirm whether these settings should be implemented or removed.
